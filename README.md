...
- **홈스크린 위젯 (스스로 움직임)**: 검은 라운드 사각형 + 하얀 두 눈. 홈 화면에서
  눈이 **폰 기울기를 따라 이리저리 보고, 가끔 깜빡**입니다. 앱을 열 필요 없이 홈에서 계속.
- **라이브 배경화면**: 홈 화면 배경 전체가 살아있는 Big Eyes.
- **탭하면 전체 화면**: 위젯을 누르면 손가락까지 따라오는 큰 눈이 열립니다.
- `minSdk 26` (Android 8.0+) — 사실상 현역 갤럭시 전 기종 지원.

> **눈 디자인**: 눈동자(검은 알)가 없는 **통짜 하얀 원**이 통째로 움직이고, 깜빡일 때는
> 하얀 작대기( — — )로 변합니다. (Nothing Playground 위젯과 동일)
