...
- **홈스크린 위젯 (스스로 움직임)**: 검은 라운드 사각형 + 하얀 두 눈. 홈 화면에서
  눈이 **스스로 이리저리 두리번거리고 표정을 바꿉니다.** 앱을 열 필요 없이 홈에서 계속.
- **표정**: 깜빡(한 번/두 번), 지그시 감은 눈( — — ), 웃는 반달눈, 윙크, 곁눈질을 랜덤하게.
- **색 테마**: 흰색 / **클로드 시그니처 주황색** — 앱에서 "색 바꾸기"로 전환 (위젯에도 즉시 반영).
- **라이브 배경화면**: 홈 화면 배경 전체가 살아있는 Big Eyes.
- **탭하면 전체 화면**: 위젯을 누르면 큰 눈이 열립니다 (화면을 만지면 잠깐 시선을 따라옴).
- `minSdk 26` (Android 8.0+) — 사실상 현역 갤럭시 전 기종 지원.

> **눈 디자인**: 눈동자(검은 알)가 없는 **통짜 원**이 통째로 움직이고, 깜빡일 때는
> 작대기( — — ), 웃을 때는 아치( ^ ^ )로 변합니다.

### 위젯이 홈에서 계속 움직이게 하는 원리 (중요)
안드로이드는 배터리 보호를 위해 일반 홈 위젯의 실시간 애니메이션을 막습니다
(Nothing 폰은 자체 런처라서 예외). 그래서 이 앱은 **아주 가벼운 포그라운드 도우미**가
위젯을 초당 몇 번 다시 그려서 살아있게 만듭니다.
- **화면이 켜져 있을 때만** 동작하고, **그림이 바뀔 때만** 갱신 → 폰이 가만히 있으면 거의 일 안 함.
- 그래서 **설치 후 앱을 딱 한 번 열어주면** 도우미가 켜지고, 이후 홈에서 계속 움직입니다.
- 삼성 배터리 최적화가 꺼버리면 → 설정에서 Big Eyes를 **"제한 없음"**으로 바꿔주세요.
...
3. 설치 완료 후:
   ...
   - **👉 앱을 한 번 실행**: 앱 서랍에서 **Big Eyes** 아이콘을 한 번 탭 → 위젯이 홈에서
     스스로 움직이기 시작합니다. (한 번만 하면 됨)
...
| 파일 | 역할 |
|------|------|
| `EyesRenderer.kt` | 눈을 그리는 공용 Canvas 렌더러 (하얀 원 이동 + 깜빡임) |
| `EyesAnimator.kt` | 시선 이징 + 깜빡임 상태 (위젯·View·배경화면 공용) |
| `BigEyesWidgetProvider.kt` | 홈스크린 위젯 (첫 프레임 렌더 + 도우미 시작/정지) |
| `BigEyesWidgetService.kt` | 위젯을 홈에서 계속 움직이게 하는 포그라운드 도우미 |
